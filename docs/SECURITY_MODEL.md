...
- **Staff**: **Amended 2026-09-17 (D-015)** — email + password held by the platform (scrypt, salted, parameters stored with the hash), accounts created by a platform admin or project manager; there is no signup. Session in an httpOnly cookie, idle timeout 12h, absolute 24h; lockout after five failed attempts with a growing window; a password change revokes every other session for that account. MFA (NFR-SEC-02) is **not yet enforced** — the columns exist and it is a deviation to close before the pilot. OIDC/SSO remains available as an additional method later; it was not adopted now because it would make staff sign-in depend on an identity provider we do not have.
