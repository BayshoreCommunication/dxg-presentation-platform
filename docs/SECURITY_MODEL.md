...
- **Staff**: **Amended 2026-09-17 (D-015)** — email + password held by the platform (scrypt, salted, parameters stored with the hash), accounts created by a platform admin or project manager; there is no signup. Session in an httpOnly cookie, idle timeout 12h, absolute 24h; lockout after five failed attempts with a growing window; a password change revokes every other session for that account. MFA (NFR-SEC-02) is **enforced** as of 2026-09-17 (D-017): TOTP, six digits, 30-second step, accepted once per step; ten single-use recovery codes issued at enrolment; a password alone yields only a five-minute challenge that grants nothing. OIDC/SSO remains available as an additional method later; it was not adopted now because it would make staff sign-in depend on an identity provider we do not have.
- **Presenters**: **Amended 2026-09-17 (D-016)** — an access code generated by DXG, used with the presenter's own email address. The same value is embedded in the emailed link, so link and typed code are one credential. Stored hashed (only a four-character hint is kept), expiring, revocable; issuing a new code retires the previous one. A code alone is insufficient without the matching email.
- **Speakers (superseded by the presenter clause above)**: passwordless magic links + one-time-code fallback. Tokens are single-purpose, bound to a specific recipient (speaker + talk scope), stored **hashed** (never plaintext), revocable, expiry ≤30 days; portal session 24h. A forwarded link grants only that speaker's scope; suspicious-use signals (many IPs/geos) surface to admins.
