...
**The blank template** is generated by `GET /events/{id}/agenda-template` from the importer's own field list, so it cannot offer a column the importer does not read or omit one it requires. It carries a banner row, headings, a format-hint row, a REQUIRED/OPTIONAL row and one example row marked `EXAMPLE`, all of which the importer treats as annotation — a downloaded template that is filled in without deleting them still imports only the real rows.
...
- A row missing a required value renders an input for that field and nothing else; filling it clears that row alone and leaves the others untouched.
