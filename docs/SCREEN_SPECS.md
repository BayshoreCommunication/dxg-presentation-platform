...
**Elements** KPI row (Rows · Incomplete rows · Warnings · New speakers); the sessions table (Row · Session · Location · Date · Time · Presenter · Action); `Download blank template`; `Import N sessions` on a file import only. **No `Track` column** (D-054): DXG's sheet has no track column and the row editor offers no track field, so it showed an em dash on every row of every agenda. A file that does carry one still creates its tracks — only the column went. **There is no `Download error report`** (D-052): it existed to carry a list of problems back to whoever produced the agenda, from a time when the only way to fix a row was to fix the file. A row is now edited in place and a row that cannot be saved is removed, so the list was a way out of a screen that no longer traps anyone.
